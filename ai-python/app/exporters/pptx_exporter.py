...
from .common import (today, done, flatten_output, executive_summary, collect_sources,
                     translate, ORANGE, ORANGE_DARK, INK, INK3)
...
def export(project, executions, lang: str = "fr") -> bytes:
    ...
    if summary:
        ...
        _box(s, translate("executive_summary", lang), 0.4, 0.4, 12.5, 0.7, 22, True, ORANGE_DARK)
    ...
    for e in done(executions):
        s = prs.slides.add_slide(blank)
        _box(s, f"{translate('heading_agent', lang)} {e.agentId} — {e.agentName}", 0.4, 0.4, 12.5, 0.7, 20, True, ORANGE_DARK)
        body = _body_text(e.output, lang)
        if len(body) > 1600:
            body = body[:1600] + "…"
    ...
    if sources:
        s = prs.slides.add_slide(blank)
        _box(s, translate("sources", lang), 0.4, 0.4, 12.5, 0.7, 20, True, ORANGE_DARK)
